...
function ListingsPage({ properties, favorites, toggleFavorite, filters, setFilters, search, setSearch }) {
  ...
  const skeletonCards = Array.from({ length: 6 }, (_, index) => (
    <div key={index} className="animate-pulse rounded-2xl border border-slate-700 bg-slate-950/60 p-6" />
  ));

  return (
    <section className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-12">
      <div className="rounded-2xl border border-slate-700 bg-slate-950 p-8 shadow-glass md:p-10">
        <div className="flex flex-col gap-6 md:items-center md:justify-between md:flex-row">
          ...
          <div className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-medium text-slate-400">
          ...
          <div className="space-y-4 rounded-2xl border border-slate-700 bg-slate-950 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <button
                ...
                className="rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:border-accent"
              >
                Reset filters
              </button>
            </div>
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {isLoading ? skeletonCards : properties.length > 0 ? properties.map((property) => {
          ...
          <div className="glass-panel col-span-full rounded-2xl p-12 text-center text-slate-400">
